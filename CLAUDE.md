...
- **Dinero = `decimal.js` vía `shared/money.ts`** (capa ÚNICA, desde 2026-06): TODO el dinero de la app (saldo, pots, apuestas, premios, pools, recompensas, precios) usa el tipo `Money` (alias de `Decimal`). NO usar `number` ni `BigInt` para dinero, NO usar `Number()` sobre saldos. API: `m(x)` construye, `add/sub/mul/div` operan (truncan a entero), `gte/gt/lte/lt/eq` comparan, `clampNonNeg` evita negativos, `toStr(x)` serializa para DB/wire, `fmt(x)` formatea display ("1.23Q"). `toBig`/`bigStr`/`addClamped` siguen como wrappers de compat pero NO usar en codigo nuevo. decimal.js instalado en server, client y raiz del repo. Motivo: con Number, 9999999999999999 - 1 = 10000000000000000 (crea dinero); con Decimal es exacto a cualquier escala.
- **Columnas monetarias = TEXT** (precision arbitraria): los INTEGER de SQLite se corrompen a REAL al escribir >2^63. Las columnas de dinero grande usan sufijo `_t TEXT` como fuente de verdad, con backfill desde la INTEGER vieja: `balance_t`, `israel_debt_t`, `israel_donation_t`, `israel_pool_t`, `hacienda_state.total_t`, `user_stats.value_t`. Se leen con `COALESCE(col_t, CAST(col AS TEXT), '0')` y se operan en JS con Decimal (NO aritmetica SQL tipo `col = col + ?`). Para stats monetarios grandes usar `maxStatBig`/`bumpStatBig`/`getStatBig` (operan sobre `value_t`); los stats de conteo (manos jugadas, etc.) siguen en `bumpStat`/`maxStat` (INTEGER).
- **Saldo NO puede ser negativo** (desde 2026-06-14, antes era "indicativo/puede ser negativo"): hay suficientes juegos que dan dinero, ya no se permite negativo. `applyBalanceDelta` clampa con `clampNonNeg(balance + delta)`. Toda apuesta/compra/buy-in debe validar `balance >= coste` ANTES de descontar y rechazar con "Saldo insuficiente". Si te quedas a 0, te fastidias. (Guards en jackpot/mines/crash/ruleta, buy-ins poker/BJ, compras tienda, desbloqueos.)
...
- Tests E2E con Playwright (`tests/e2e.mjs`, 19 tests). Ejecutar tras cambios significativos (ver sección Sistema de tests E2E).
