...
exports.resumeBlindTimers = exports.findActiveRoomForUser = exports.restartTournament = exports.checkTournamentEnd = exports.markBustedPlayers = exports.startBlindEscalation = exports.nextHand = exports.endRound = exports.resolveRoundSync = exports.advanceStreet = exports.bettingClosed = exports.contenders = exports.gatherBetsToPot = exports.handlePlayerAction = exports.startGame = exports.rebuy = exports.leaveRoom = exports.joinRoom = exports.getRoom = exports.evictAll = exports.touchRoom = exports.restoreRoom = exports.createRoom = exports.getRooms = exports.clearBlindTimer = exports.bumpMaxChips = void 0;
...
const closePlayerSession = (room, player, cashOutChips) => {
    ...
    (0, db_1.recordMatchHistory)(player.userId, room.name, buyIn, maxChips, cashOutChips, Date.now()).catch(e => console.error('Error guardando historial de partida:', e));
...
const bumpMaxChips = (room) => {
    room.players.forEach(p => {
        if (!p.isActive)
            return;
        if (p.sessionBuyIn == null)
            return;
        const cur = p.sessionMaxChips ?? 0;
        if (p.chips > cur)
            p.sessionMaxChips = p.chips;
    });
};
exports.bumpMaxChips = bumpMaxChips;
const rooms = new Map();
// Timers de escalado de ciegas (solo salas en modo torneo)
const blindTimers = new Map();
const clearBlindTimer = (roomId) => {
    const t = blindTimers.get(roomId);
    if (t) {
        clearTimeout(t);
        blindTimers.delete(roomId);
    }
};
exports.clearBlindTimer = clearBlindTimer;
const getRooms = () => {
...
const createRoom = (id, name, persistent = false, tierIndex = 0, blindDivisor = pokerEngine_1.DEFAULT_BLIND_DIVISOR, blindLevelDuration = 0) => {
    const buyIn = types_1.STAKE_TIERS[tierIndex] ?? types_1.STAKE_TIERS[0];
    const { smallBlind, bigBlind } = (0, types_1.blindsFor)(buyIn, blindDivisor);
    const newRoom = {
        id,
        name,
        players: [],
...
const evictAll = (roomId) => {
    ...
    if (!room.persistent) {
        (0, exports.clearBlindTimer)(roomId);
        rooms.delete(roomId);
        (0, db_1.deleteRoomFromDB)(roomId).catch(e => console.error('DB delete error', e));
        return cashOuts;
    }
...
const joinRoom = (roomId, player) => {
    ...
    if (existing && existing.hasCashedOut) {
        if (room.players.filter(p => p.isActive).length >= 8)
            return 'full';
...
const leaveRoom = (roomId, socketId) => {
    ...
    if (isInHand) {
        ...
        if (signal !== 'continue') {
            (0, exports.resolveRoundSync)(room);
        }
    }
...
const rebuy = (roomId, userId, buyIn) => {
    const room = rooms.get(roomId);
    if (!room)
        return false;
    const player = room.players.find(p => p.userId === userId);
    if (!player || player.chips > 0)
        return false; // Solo se recompra estando a 0
    player.chips = buyIn;
    ...
    if (buyIn > (player.sessionMaxChips ?? 0))
        player.sessionMaxChips = buyIn;
...
const startGame = (roomId) => {
    ...
    if (!room || room.players.filter(p => p.isActive && p.chips > 0).length < 2)
        return false;
    room.deck = (0, pokerEngine_1.createDeck)();
    (0, pokerEngine_1.shuffleDeck)(room.deck);
    room.communityCards = [];
    room.pot = 0;
    room.highestBet = 0;
    room.winners = [];
    ...
    (0, pokerEngine_1.dealCards)(room); // This also resets hasFolded, hasActed, currentBet
    (0, pokerEngine_1.updateHandNames)(room);
    room.phase = 'preflop';
    ...
    (0, exports.startBlindEscalation)(room.id);
    return true;
};
exports.startGame = startGame;
const handlePlayerAction = (roomId, userId, actionType, amount) => {
    const room = rooms.get(roomId);
    ...
    if (actionType === 'Fold') {
        ...
        if (toCall > 0)
            return false; // No puede hacer check si debe igualar
    }
        ...
        if (raiseTotal <= room.highestBet && raiseTotal < player.chips + player.currentBet)
            return false;
...
const advanceTurn = (room) => {
    ...
    let count = 0;
    while (count < numPlayers && (!room.players[nextIndex].isActive ||
...
const advanceStreet = (room) => {
    (0, exports.gatherBetsToPot)(room);
...
const resolveRoundSync = (room) => {
    for (let guard = 0; guard < 8; guard++) {
        if ((0, exports.contenders)(room).length <= 1) {
            (0, exports.endRound)(room);
            return;
        }
        if (room.phase === 'showdown') {
            (0, exports.endRound)(room);
        ...
        if (room.phase === 'river') {
            (0, exports.advanceStreet)(room);
            (0, exports.endRound)(room);
            return;
        }
        (0, exports.advanceStreet)(room);
        if (!(0, exports.bettingClosed)(room))
            return; // aún se puede apostar con normalidad
    }
};
exports.resolveRoundSync = resolveRoundSync;
const endRound = (room) => {
    (0, exports.gatherBetsToPot)(room);
    ...
    if (activePlayers.length === 1) {
        ...
        while (remainingPot > 0 && playersForPot.length > 0) {
            ...
            if (currentSidePot > 0) {
                ...
                const tempRoom = { ...room, players: eligiblePlayers };
                const evaluatedWinnersInfo = (0, pokerEngine_1.evaluateHands)(tempRoom);
                ...
                evaluatedWinnersInfo.forEach((wInfo) => {
...
const startBlindEscalation = (roomId) => {
    const room = rooms.get(roomId);
    if (!room || !room.isTournament || !room.blindLevelDuration)
        return;
    if (room.blindLevelStartedAt)
        return; // ya en marcha
    room.blindLevelStartedAt = Date.now();
    scheduleBlindEscalation(roomId);
};
exports.startBlindEscalation = startBlindEscalation;
const scheduleBlindEscalation = (roomId) => {
    const room = rooms.get(roomId);
    if (!room || !room.isTournament || !room.blindLevelDuration)
        return;
    (0, exports.clearBlindTimer)(roomId);
...
const escalateBlinds = (roomId) => {
    const room = rooms.get(roomId);
    if (!room || !room.isTournament || room.tournamentEnded)
        return;
    const nb = (0, types_1.nextBlinds)(room.bigBlind);
...
const markBustedPlayers = (roomId) => {
    const room = rooms.get(roomId);
...
exports.restartTournament = restartTournament;
const findActiveRoomForUser = (userId) => {
...
exports.findActiveRoomForUser = findActiveRoomForUser;
const resumeBlindTimers = () => {
...
exports.resumeBlindTimers = resumeBlindTimers;
