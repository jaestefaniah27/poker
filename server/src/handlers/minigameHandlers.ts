...
import { claimDailyBonus, claimHourlyBonus, getUser, toPublicUser, applyBalanceDelta, recordJackpotSpin, claimFreeSpins, useFreeSpin as consumeFreeSpin, setJackpotUnlockLevel, spendLevelPoint, addXp, parsePools, addHaciendaTotal, bumpStat, maxStat } from '../db';
...
export const minigameHandlers = (socket: Socket) => {
  ...
  socket.on('minesReveal', async ({ token, cell }, callback) => {
    ...
    if (revealed === safeTiles) {
      ...
      bumpStat(user.id, 'mines_total_won', winnable);
      maxStat(user.id, 'mines_biggest_win', winnable);
  ...
  socket.on('minesCashout', async ({ token }, callback) => {
    ...
    await addXp(user.id, xpWin);
    bumpStat(user.id, 'mines_cashouts');
    bumpStat(user.id, 'mines_total_won', winAmount);
    maxStat(user.id, 'mines_biggest_win', winAmount);
    maxStat(user.id, 'mines_best_mult_x100', Math.round(multiplier * 100));
    const dbUser = await getUser(user.id);
